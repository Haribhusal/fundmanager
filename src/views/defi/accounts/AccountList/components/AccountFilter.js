...
const FilterForm = forwardRef(({ onSubmitComplete }, ref) => {
    ...
    const filterData = useSelector(
        (state) => state.defiAccountList.data.filterData
    )
    const sortedColumn = useSelector(
        (state) => state.defiAccountList.state.sortedColumn
    )

    const handleSubmit = (values) => {
        onSubmitComplete?.()
        dispatch(setFilterData(values))
        dispatch(getAccounts(initialTableData))
            ...
            {({ values, touched, errors }) => (
                <Form>
                    <FormContainer>
                        ...
                        >
                            <h6 className="mb-4">Account Category</h6>
                            <Field name="category">
                                {({ field, form }) => (
                                    <>
                                        <Checkbox.Group
